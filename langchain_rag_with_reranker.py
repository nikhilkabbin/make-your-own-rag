...
# reranking utilities
from langchain_text_splitters import ContextualCompressionRetriever
from langchain_text_splitters.document_compressors import LLMChainExtractor
...
class ICCTournamentRAG:
    ...
    def setup_chain(self):
        ...
        # Create compressor (uses LLM to re-score)
        compress_prompt = PromptTemplate.from_template(
            "Identify the most relevant parts of this document for the query: {query}"
        )
        compressor = LLMChainExtractor.from_llm(self.llm, compress_prompt)
        ...
        # Use reranker instead of regular retriever
        self.chain = (
